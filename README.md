...
Open [http://ec2-13-58-19-50.us-east-2.compute.amazonaws.com:3000](http://ec2-13-58-19-50.us-east-2.compute.amazonaws.com:3000) to view it in the browser.
...
```js
  "proxy": "http://ec2-13-58-19-50.us-east-2.compute.amazonaws.com:4000",
```

This way, when you `fetch('/api/todos')` in development, the development server will recognize that it’s not a static asset, and will proxy your request to `http://ec2-13-58-19-50.us-east-2.compute.amazonaws.com:4000/api/todos` as a fallback. The development server will only attempt to send requests without a `text/html` accept header to the proxy.

Conveniently, this avoids [CORS issues](http://stackoverflow.com/questions/21854516/understanding-ajax-cors-and-security-considerations) and error messages like this in development:

```
Fetch API cannot load http://ec2-13-58-19-50.us-east-2.compute.amazonaws.com:4000/api/todos. No 'Access-Control-Allow-Origin' header is present on the requested resource. Origin 'http://ec2-13-58-19-50.us-east-2.compute.amazonaws.com:3000' is therefore not allowed access. If an opaque response serves your needs, set the request's mode to 'no-cors' to fetch the resource with CORS disabled.
```
...
If you use routers that use the HTML5 [`pushState` history API](https://developer.mozilla.org/en-US/docs/Web/API/History_API#Adding_and_modifying_history_entries) under the hood (for example, [React Router](https://github.com/ReactTraining/react-router) with `browserHistory`), many static file servers will fail. For example, if you used React Router with a route for `/todos/42`, the development server will respond to `ec2-13-58-19-50.us-east-2.compute.amazonaws.com:3000/todos/42` properly, but an Express serving a production build as above will not.
...
HOST | :white_check_mark: | :x: | By default, the development web server binds to `ec2-13-58-19-50.us-east-2.compute.amazonaws.com`. You may use this variable to specify a different host.
