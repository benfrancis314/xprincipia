...
export default class WelcomeUserUnit extends React.Component {
    ...
    componentDidMount(){
        var self = this;
        // window.scrollTo(0,0);
        return axios.get( Config.API + '/resources/feed').then(function (response) {
            self.setState({
                feedResources: response.data.reverse(),
                feedResources: response.data.reverse().slice(0,10)
            })
        }) 
    }
	render() {
        ...
        <div id="feedContainer">
            ...
            {React.cloneElement(this.props.children, {resources: this.state.feedResources})}
            <div id="feedBottom">
                <br />
            </div>
        </div>
