...
export default class WelcomeUserUnit extends React.Component {
    ...
    hoverLabel() {
        $(document).ready(function() {
                $('#feedTitle').html("new resources").fadeIn(7500);
                $('#feedTitle').attr('id','feedTitleHover');
                $('#feedBottom').attr('id','feedBottomBlue');
        });
    }
	...
	render() {
		...
		return (
            <div id="feedUnitContainer">
                <div id="feedListDiv">
                    <ul id="feedUnitList"> 
                        {this.props.resources.map(this.renderItem)}
                    </ul>	 
                </div>
...
if (resource.Private === true) {
        return (
            <div key={resource.ID} id="nodisplay">
            </div>
        );
// We may want to show questions that are on proposals too
} else if (resource.ParentType === 1) {
    ...
    //         </div>
      ...
      return (
        <li key={resource.ID} id="feedListUnit">
