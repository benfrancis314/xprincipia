...
updateNotebook() {
  ...
  var self = this;
  axios.put( Config.API + '/auth/notebooks/update?id='+String(self.state.notebook.ID), {
      username: cookie.load('userName'),
      title : self.state.title,
      summary : self.state.summary,
      description : self.state.description,
      sources: self.state.references
    })
...
hoverCloseNotebook() {
  $(document).ready(function() {
      $('#notebookUnitHeader').html('<span id="brightWhite">save and </span>exit').fadeIn(7500);
      ...
      return (
        <div id="notebookFullContainer">
          ...
          {/* {this.props.currentNotebook} */}
            <input id="notebookFullTitle" placeholder="notebook title" type="text" onChange={this.updateNotebook}></input>
            <textarea id="notebookFullContent" placeholder="Brainstorm or record your thoughts. " autoFocus onChange={this.updateNotebook}></textarea>
            <div id="notebookFullSourcesTitle">
                sources
            </div>
            <textarea id="notebookFullResources" onChange={this.updateNotebook} placeholder="Add sources here"></textarea>
